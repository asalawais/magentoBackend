    ...
    public function execute()
    {
        ...
        if($this->getRequest()->isAjax()) {
            ...
            try {
                ...
                $this->galleryArray->execute();
                $this->loggerFile->info('gallery called: ');
                //if($products) {
                    ...
                    foreach (range(0, 2000, 50) as $skip) {
                        $take = $c + $skip;
                        $this->loggerFile->info($take);
                        $xmlPath = $var . 'import/afas_images' . $take . '.xml';
                        $this->loggerFile->info('xmlPath: '.$xmlPath);
    ...
    public function getConfigurableProducts() {
        ...
        foreach($collection as $confProduct){
            if($confProduct->getTypeId() == "configurable"){
                $productConf = $this->_productRepositoryFactory->getById($confProduct->getId());
                $confImages = [];
                $children = $confProduct->getTypeInstance()->getUsedProducts($confProduct);
                if(count($children)>0){
                    foreach ($children as $child){

                        $product = $this->_productRepositoryFactory->getById($child->getId());
