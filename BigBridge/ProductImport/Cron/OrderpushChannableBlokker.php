 ...
 */
class OrderpushChannableBlokker
{
    ...
    public function getOrders()
   {
    ...
    private function getClient(){
       ...
       $this->logger->info('OrderPush Channable Blokker: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->info('OrderPush Channable Blokker: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush channable Blokker:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush channable Blokker: result'.$suc);
            ...
            $this->logger->critical('OrderPush channable Blokker: '.$e->getMessage());
        }
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "Blokker",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => $data['Fields']['channelId'],
                    'U1894E7EC43E51449CDEF17ABAA571DD5' => $data['Fields']['tracking']
