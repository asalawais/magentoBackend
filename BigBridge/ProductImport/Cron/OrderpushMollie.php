...
class OrderpushMollie //implements CronJobInterface
{
     ...
     */
     ...
     public function __construct(
        ...
        \Magmodules\Channable\Model\Order\ResourceModel\CollectionFactory $orderChannable,
    ...
    public function getOrders()
   {
    ...
    private function getClient(){
            $this->logger->info('OrderPush Mollie: start');
            $client = $this->_restCurlClient->getClient();
            $OrderData = [];
            $this->logger->info('OrderPush Mollie: start count' . count($this->getOrders()));
            foreach ($this->getOrders() as $order) {
                try {
                $this->logger->info('OrderPush Mollie: order number' . $order->getRealOrderId());
                ...
                foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                    ...
                    //echo $FbSales;
                    $this->logger->critical('OrderPush Mollie:' . $FbSales);
                    $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
                    $dataResult = json_decode($suc);
                    $this->logger->critical('OrderPush Mollie: result' . $suc);
                    $this->logger->critical('OrderPush Mollie Packing Slip start');
                    ...
                    $this->logger->critical('OrderPush Mollie: GUID result' . print_r($guidResult, true));
                    ...
                    $this->logger->critical('OrderPush Mollie: result' . print_r($packingSlips, true));
                ...
                if ($orderN->getId() && !empty($afasOrder)) {
                    ...
                    $this->logger->critical('OrderPush Mollie: '.$e->getMessage());
                }
        }
...
echo count($order->getAllVisibleItems());
            foreach ($order->getAllVisibleItems() as $item) {
                //if ($item->getSku() != '8718247575039') {
                    $data['Fields'] = array(
                        "ItCd" => $item->getSku(),
                        "QuUn" => $item->getQtyOrdered(),
                        "VaIt" => $this->getTypeItem($item->getSku()),
                        "BuUn" => "STK"
                    );
                    $sourceCode = 'default';
                    $sourceItemList = $this->getSourceItemBySku($item->getSku());

                    $unit = $this->getAfasUnit($item->getSku());
                    //$preferredWarehouse = $this->getPreferredWarehouse($item->getProductId());
                    ...
                    //$sourceCode = 'default';
                //disabled for test
                    $sourceCode = $this->getPreferredWareHouse($item->getSku());
