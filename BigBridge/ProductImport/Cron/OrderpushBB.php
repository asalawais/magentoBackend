 ...
 */
class OrderpushBB //implements CronJobInterface
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'boschbedding')
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush BB: start');
        ...
        $OrderData = [];
        $this->logger->info('OrderPush BB: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                ...
                $this->logger->info('OrderPush BB: GUID result' . print_r($guidResult, true));
                $OrderData['packing_time'] = $this->timezone->date(new \DateTime())->format('Y-m-d');
                $packingSlipResult = $this->curlHelper->getPackingSlip($dataResult->results->FbSales->OrNu, $OrderData, 10222);
                $packingSlips[] = $packingSlipResult['results']['FbDeliveryNote']['OrNu'];
                $this->logger->info('OrderPush BB: result' . print_r($packingSlips, true));
            ...
            if ($orderN->getId() && !empty($afasOrder)) {
                ...
                $this->logger->critical('OrderPush BB: '.$e->getMessage());
            }
     }
