 ...
 */
class OrderpushShopifyOmraBedding
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('shopify_order_id', array("null" => false))
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush OmraBedding: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush OmraBedding: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush Omrabedding:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush Omrabedding: result'.$suc);
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' || $countryCode == 'FR' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "omrabedding",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => 'Omrabedding',  // Channel id
