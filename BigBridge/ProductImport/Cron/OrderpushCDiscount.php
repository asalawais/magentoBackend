 ...
 */
class OrderpushCDiscount //implements CronJobInterface
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'cdiscount')
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Cdiscount: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Cdiscount: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                ...
                $this->logger->critical('OrderPush Cdiscount: result' . $suc);
            ...
            if ($orderN->getId() && !empty($afasOrder)) {
                ...
                $this->logger->critical('OrderPush Cdiscounte: '.$e->getMessage());
            }
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' || $countryCode == 'FR' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "Cdiscount",
