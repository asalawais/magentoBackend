...
class OrderpushEmesa
{
    ...
    public function getOrders()
   {
       ...
       //->addFieldToFilter('status', array("in" => ['processing', 'complete']))
       //->addFieldToFilter('entity_id', array("eq" => 4)) //for testing
       //->addFieldToFilter('state', array("in" => ['processing', 'complete']))
       ...
       $this->logger->info('OrderPush Emesa: '.count($collection));
       return $collection;
    }
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Emesa: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Emesa: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush Cdiscount:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush Emesa: result'.$suc);
                ...
                $packingSlipResult = $this->curlHelper->getPackingSlip($dataResult->results->FbSales->OrNu, $OrderData, 10258);
            ...
            $this->logger->critical('OrderPush Emesa: '.$e->getMessage());
        }
 ...
 public function getOrderItems($order, $products){
            ...
            foreach ($order->getAllVisibleItems() as $item)
            {
               $data['Fields'] = array(
                                    ...
                                    "Upri" => number_format(($item->getPrice() / 1.21), 2, '.', ''),
               ...
               $products['Element'][$sourceCode][] = $data;
                 ...
                 $data['Fields'] = array(
                     "ItCd" => 'TR-DS',
                     "QuUn" => 1,
                     "Upri" => number_format(($order->getShippingAmount() / 1.21), 2, '.', ''),
                     "VaIt" => 2,
                     "BuUn" => "STK"
                 );
                 $sourceCode = $this->getPreferredWareHouse($item->getProductId());
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' || $countryCode == 'FR' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U2763E2004660BC85D24EF49A1BF586A8' => "",
                    'U2A00775A4471FF87D4EA1E9826C973B0' => true,
                    'U4949642146F96C50B8900BBD2247E587' => "Emesa",
