 ...
 */
class OrderpushGroupon //implements CronJobInterface
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'groupon')
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Groupon: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Groupon: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush Groupon:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush Groupon: result'.$suc);
            ...
            $this->logger->critical('OrderPush Groupon: '.$e->getMessage());
        }
 ...
 public function getOrderInfo($order, $orderInfo){
            ...
            $orderInfo['afasRefNbr'] = $order->getData('order_ref_nbr');
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     $house = isset($data['Fields']['street'][1]) ? $data['Fields']['street'][1] : '';
     $houseExt = isset($data['Fields']['street'][2]) ? $data['Fields']['street'][2] : '';
      $dataAfas['Element']['Fields'] = [
                    ...
                    'UCC40DB5C4721EEB3497F9FBE4BEF7EB7' => $data['Fields']['street'][0] . ' '. $house. ' ' . $houseExt,
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "GroupOn",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => $data['Fields']['afasRefNbr'],  // Channel id
