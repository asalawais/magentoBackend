 ...
 */
class OrderpushChannableAmazon //implements CronJobInterface
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'amazon')
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Channable Amazon: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Channable Amazon: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush channable Amazon:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush channable Amazon: result'.$suc);
 ...
 public function getOrderInfo($order, $orderInfo){
            ...
            //$orderInfo['channelId'] = $order->getChannelId();  // disabled for manual import
            $orderInfo['channelId'] = '';
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "AMAZON",
