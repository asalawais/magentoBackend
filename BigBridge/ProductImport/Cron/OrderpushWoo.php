...
class OrderpushWoo
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'woo_connect')
           ->addFieldToFilter('sherpa_token', array("neq" => 'NULL'))
           ->addFieldToFilter('sherpa_order_number', array("neq" => 'NULL'))
           ->addFieldToFilter('sherpa_error_code', array("null" => true))
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Woo: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Woo: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush Woo:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush Woo: result'.$suc);
            ...
            $this->logger->critical('OrderPush Woo: '.$e->getMessage());
        }
     }
 ...
 public function getOrderInfo($order, $orderInfo){
            ...
            $orderInfo['RfCs'] = $orderRef->getData('channel_label');;
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => $data['Fields']['RfCs'],
                    'U5E210D8E41377C0448CFFFABEF9A9959' => $data['Fields']['RfCs'],  // Channel id
