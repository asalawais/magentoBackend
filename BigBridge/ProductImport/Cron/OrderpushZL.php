...
class OrderpushZL
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('channel_name', 'zittenenliggen')
       ->addFieldToFilter('store_id', 10)
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush ZL: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->info('OrderPush ZL: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                ...
                //echo $FbSales;
                $this->logger->critical('OrderPush ZL:' . $FbSales);
                $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
                $dataResult = json_decode($suc);
                $this->logger->critical('OrderPush ZL: result' . $suc);
                ...
                $this->logger->info('OrderPush ZL: GUID result' . print_r($guidResult, true));
                $OrderData['packing_time'] = $this->timezone->date(new \DateTime())->format('Y-m-d');
                $packingSlipResult = $this->curlHelper->getPackingSlip($dataResult->results->FbSales->OrNu, $OrderData, 10397);
                $packingSlips[] = $packingSlipResult['results']['FbDeliveryNote']['OrNu'];
                $this->logger->info('OrderPush ZL: result' . print_r($packingSlips, true));
            ...
            if ($orderN->getId() && !empty($afasOrder)) {
                ...
                $this->logger->critical('OrderPush ZL: '.$e->getMessage());
            }
     }
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "ZL",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => 'Zitten & Liggen',  // Channel id
