...
class OrderpushChannableTest
{
    ...
    public function getOrders()
   {
       ...
       ->addFieldToFilter('entity_id', array("eq" => 15819))
    ...
    private function getClient(){
            $this->logger->info('OrderPush Channable: start');
            $client = $this->_restCurlClient->getClient();
            $OrderData = [];
            $this->logger->info('OrderPush Channable: start count' . count($this->getOrders()));
            foreach ($this->getOrders() as $order) {
                try {
                    echo $order->getRealOrderId();
                $this->logger->info('OrderPush Channable: order number' . $order->getRealOrderId());
                ...
                foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                    ...
                    //echo $FbSales;
                    $this->logger->critical('OrderPush channable:' . $FbSales);
                    $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
                    $dataResult = json_decode($suc);
                    $this->logger->critical('OrderPush channable: result' . $suc);
                    $this->logger->critical('OrderPush channable Packing Slip start');
                    ...
                    print_r($OrderData['Objects']);
                    $this->logger->critical('OrderPush channable: lines Objects result' . print_r($OrderData['Objects'], true));
                ...
                if ($orderN->getId() && !empty($afasOrder)) {
                    ...
                    $this->logger->critical('OrderPush channable: '.$e->getMessage());
                }
        }
