 ...
 */
class OrderpushNielsSlaapcomfort //implements CronJobInterface
{
    ...
    public function getOrders()
   {
       ...
       ->addAttributeToSelect('*')
           ...
           ->addFieldToFilter('entity_id', array("gt" => 15553)) //for testing
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush NS: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->info('OrderPush NS: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
                ...
                //echo $FbSales;
                $this->logger->critical('OrderPush NS:' . $FbSales);
                $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
                $dataResult = json_decode($suc);
                $this->logger->critical('OrderPush NS: result' . $suc);
                ...
                $this->logger->info('OrderPush NS: GUID result' . print_r($guidResult, true));
                $OrderData['packing_time'] = $this->timezone->date(new \DateTime())->format('Y-m-d');
                $packingSlipResult = $this->curlHelper->getPackingSlip($dataResult->results->FbSales->OrNu, $OrderData, 10411);
                $packingSlips[] = $packingSlipResult['results']['FbDeliveryNote']['OrNu'];
                $this->logger->info('OrderPush NS: result' . print_r($packingSlips, true));
            ...
            if ($orderN->getId() && !empty($afasOrder)) {
                ...
                $this->logger->critical('OrderPush NS: '.$e->getMessage());
            }
     }
 ...
 public function getOrderInfo($order, $orderInfo){
            ...
            $orderInfo['RfCs'] = 'Niels slaapcomfort';
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "nielsslaapcomfort",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => 'Niels slaapcomfort',  // Channel id
