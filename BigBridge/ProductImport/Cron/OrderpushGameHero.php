...
class OrderpushGameHero
{
    ...
    public function getOrders()
   {
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush GH: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush GH: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush GH:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush GH: result'.$suc);
                ...
                $this->logger->critical('OrderPush Gamehero: GUID result' . print_r($guidResult, true));
                $OrderData['packing_time'] = $this->timezone->date(new \DateTime())->format('Y-m-d');
                $packingSlipResult = $this->curlHelper->getPackingSlip($dataResult->results->FbSales->OrNu, $OrderData, 10289);
                $packingSlips[] = $packingSlipResult['results']['FbDeliveryNote']['OrNu'];
                $this->logger->critical('OrderPush Gamehero: result' . print_r($packingSlips, true));
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "gamehero",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => 'GameHero',  // Channel id
