 ...
 */
class OrderpushHomeDeco //implements CronJobInterface
{
    ...
    public function getOrders()
   {
    ...
    private function getClient(){
       ...
       $this->logger->debug('OrderPush Homedeco: start');
        $client = $this->_restCurlClient->getClient();
        $OrderData = [];
        $this->logger->debug('OrderPush Homedeco: start count'.count($this->getOrders()));
...
//print_r($OrderData);
            ...
            foreach ($OrderData['Objects']['Element'] as $sourceKey => $items) {
               ...
               //echo $FbSales;
               $this->logger->critical('OrderPush HomeDeco:'.$FbSales);
               $suc = $client->callAfas('POST', 'connectors/FbSales', [], $FbSales);
               $dataResult = json_decode($suc);
               $this->logger->critical('OrderPush HomeDeco: result'.$suc);
            ...
            $this->logger->critical('OrderPush HomeDeco: '.$e->getMessage());
        }
     }
 ...
 public function setFbSales($data, $dataAfas, $sourceKey){
     ...
     if($countryCode == 'DE' || $countryCode == 'BE' || $countryCode == 'FR' ) {
      ...
      $dataAfas['Element']['Fields'] = [
                    ...
                    'U4949642146F96C50B8900BBD2247E587' => "HomeDeco",
                    'U5E210D8E41377C0448CFFFABEF9A9959' => '',  // Channel id
