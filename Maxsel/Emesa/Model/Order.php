...
namespace Maxsel\Emesa\Model;

use Magmodules\Channable\Helper\General as GeneralHelper;
use Maxsel\Emesa\Helper\OrderGeneral as OrderlHelper;
...
use Maxsel\OrderImport\Model\Config;
use Maxsel\OrderImport\Service\Order\CreateInvoice;
use Maxsel\OrderImport\Service\Order\AddressData;
use Maxsel\OrderImport\Service\Order\Items\Add as AddItems;
...
class Order {
    ...
    public function importOrder($data, $storeId)
    {
        ...
        try {
            ...
            $this->checkoutSession->setHomeShoppingEnabled(1);
            $this->checkoutSession->setHomeShipping($shippingPriceCal);

            $shippingMethod = $this->getShippingMethod($cart, $store, $itemCount, $shippingPriceCal);
            $this->generalHelper->addTolog('importOrder: ' . $data['emesa_id'], $shippingMethod);
           ...
           /* if ($this->lvb && $this->orderHelper->getLvbSkipStock($store->getId())) {
            ...
            if ($this->orderHelper->getUseChannelOrderId($storeId)) {
                $newIncrementId = $this->orderHelper->getUniqueIncrementId($data['emesa_id'], $storeId);
            ...
            $this->connectHelper->putOrderConfirm($data['ref']);
            $this->generalHelper->addTolog('importOrderRef: ' . $data['ref'], $data['ref']);
            $order->setData('order_ref_nbr', $data['ref']);
            $order->setData('order_note', $data['note']);
            ...
            $this->generalHelper->addTolog('importOrder: ' . $data['emesa_id'], $e->getMessage());
            return $this->jsonRepsonse($e->getMessage(), '', $data['emesa_id']);
        } finally {
    ...
    private function getShippingMethod($cart, $store, $itemCount, $shippingPriceCal)
    {
        ...
        $shippingMethod = 'homeshopping_homeshopping';
        ...
        if ($result) {
            ...
            if ($shippingMethod != 'homeshopping_custom') {
