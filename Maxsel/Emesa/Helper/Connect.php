    ...
    public function putProductByMarketCategoryId($productId)
    {
        ...
        $this->logger->info('Result product data: ' . print_r($product->getData(), true));
                if ($product->getDescription()) {
                    ...
                    $this->logger->info('Exception when calling ProductsApi->putProduct: product description is empty');
                    return false;
                }
    }
...
// Configure OAuth2 access token for authorization: ClientCredentials
        ...
        try {
            ...
            $this->logger->info('Exception when calling OrdersApi->listOrders: '. $e->getMessage());
        }
    }
