...
namespace FIRSTSHOW\Portfolio\Controller;
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
		...
		if(count($arrRouter)==1){
			...
			$this->_eventManager->dispatch(
				'firstshow_portfolio_controller_router_match_before',
			...
			if(count($info) == 1){
				$category  = $this->getModel('FIRSTSHOW\Portfolio\Model\Category')
					->getCollection()
					->addFieldToFilter('identifier', $info[0])
					->getFirstItem();
				
				$portfolio  = $this->getModel('FIRSTSHOW\Portfolio\Model\Portfolio')
					->getCollection()
					->addFieldToFilter('identifier', $info[0])
