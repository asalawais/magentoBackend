...
class Savecategory extends \FIRSTSHOW\Portfolio\Controller\Adminhtml\Portfolio
{
    /**
     * Save action
     *
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $model = $this->_objectManager->create('FIRSTSHOW\Portfolio\Model\Category')->load($id);
            if (!$model->getId() && $id) {
			...
			if($identifier!=''){
				$searchCat = $this->_objectManager->create('FIRSTSHOW\Portfolio\Model\Category')
					->getCollection()
				...
				$searchPortfolio = $this->_objectManager->create('FIRSTSHOW\Portfolio\Model\Portfolio')
					->getCollection()
					->addFieldToFilter('identifier', $identifier);
