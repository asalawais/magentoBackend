 ...
 */
namespace FIRSTSHOW\Portfolio\Model\ResourceModel;
...
class Portfolio extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('firstshow_portfolio_items', 'portfolio_id');
    }
	...
	protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        if(isset($_FILES['thumbnail_image']['name']) && $_FILES['thumbnail_image']['name'] != '') {
			...
			$path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('firstshow/portfolio/thumbnail/');
			$uploader->save($path);
		...
		if(isset($_FILES['base_image']['name']) && $_FILES['base_image']['name'] != '') {
			...
			$path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('firstshow/portfolio/image/');
			$uploader->save($path);
			$fileName = $uploader->getUploadedFileName();
			if ($fileName) {
		...
		$table = $this->getTable('firstshow_portfolio_category_items');
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('firstshow_portfolio_category_items'), $condition);
    ...
    public function lookupCategoryIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('firstshow_portfolio_category_items'),
            'category_id'
        )->where(
	...
	public function joinFilter($collection, $categoryId){
		$tableName = $this->getTable('firstshow_portfolio_category_items');
	...
	public function getRelatedPortfolio($portfolios, $id, $catString){
		$storeTable = $this->getTable('firstshow_portfolio_category_items');
