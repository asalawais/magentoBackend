...
class Save extends Blog
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                $model = $this->_objectManager->create('FIRSTSHOW\Blog\Model\Category');
                ...
                if (!isset($data['category']['url_key']) || $data['category']['url_key'] == '') {
                    ...
                    $existUrlKeys = $this->_objectManager->create('FIRSTSHOW\Blog\Model\Category')->getCollection()->addFieldToFilter('url_key', ['eq' => $urlKey]);
