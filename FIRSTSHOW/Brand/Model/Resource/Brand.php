...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('firstshow_brand', 'brand_id');
    }

    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('firstshow_brand_product'), $condition);
        $this->getConnection()->delete($this->getTable('firstshow_brand_store'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('firstshow_brand_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['firstshow_brand_store' => $this->getTable('firstshow_brand_store')],
                $this->getMainTable() . '.brand_id = firstshow_brand_store.brand_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'firstshow_brand_store.store_id IN (?)',
                $storeIds
            )->order(
                'firstshow_brand_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('firstshow_brand_store')],
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('firstshow_brand_store'),
            'store_id'
        )->where(
            'brand_id = ?',
            (int)$brandId
    ...
    public function lookupProductIds($brandId)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('firstshow_brand_product'),
            'product_id'
        )->where(
            'brand_id = ?',
            (int)$brandId
