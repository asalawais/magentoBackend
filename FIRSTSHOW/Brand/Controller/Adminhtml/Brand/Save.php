...
class Save extends \FIRSTSHOW\Brand\Controller\Adminhtml\Brand
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                ...
                $model = $this->_objectManager->create('FIRSTSHOW\Brand\Model\Brand');
                ...
                if (isset($_FILES['small_image']['name']) && $_FILES['small_image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        $path = $this->_objectManager->get('Magento\Framework\Filesystem')->getDirectoryRead(DirectoryList::MEDIA)
                            ->getAbsolutePath('firstshow_brand/');
                        $uploader->save($path);
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['brand']['small_image'] = 'firstshow_brand' . $fileName;
                        }
                    } else {
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['brand']['image'] = 'firstshow_brand' . $fileName;
                        }
                    } else {
                        $this->messageManager->addError(__('Disallowed file type.'));
                        return $this->redirectToEdit($model, $data);
                ...
                $model->save();
                $brand = $this->_objectManager->create('FIRSTSHOW\Brand\Model\Brand')->load($model->getId());
                $optionId = $this->saveOption('firstshow_brand', $brand->getName(), (int)$brand->getOptionId());
                $brand->setOptionId($optionId);
                $brand->save();
                if (isset($data['product_ids']) && ($data['product_ids'] != '' || $data['product_ids'] != null)) {
                    $productIdsInBrand = array();
                    $productCollection = $this->_objectManager->create('FIRSTSHOW\Brand\Model\Product')->getCollection();
                    ...
                    $productsDelete = $this->_objectManager->create('FIRSTSHOW\Brand\Model\Product')->getCollection()
    ...
    {
        return $this->_authorization->isAllowed('FIRSTSHOW_Brand::save_brand');
    }
