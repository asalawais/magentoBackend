...
namespace FIRSTSHOW\Mpanel\Helper\Swatches;
...
class Data extends \Magento\Swatches\Helper\Data
{
    ...
    private function getAllSizeImages(ModelProduct $product, $imageFile)
    {
		...
		//echo '<pre>'; var_dump($largeSize); echo '<br>';
		//echo '<pre>'; var_dump($mediumSize); echo '<br>';
		//echo '<pre>'; var_dump($minSize); echo '<br>';
        $array= [
            'large' => $this->imageHelper->init($product, 'product_swatch_image_large')
				->resize($largeSize['width'],$largeSize['height'])
                ->constrainOnly(true)->keepAspectRatio(true)->keepFrame(true)
                ->setImageFile($imageFile)
                ->getUrl(),
            'medium' => $this->imageHelper->init($product, 'product_swatch_image_medium')
				->resize($mediumSize['width'],$mediumSize['height'])
                ->constrainOnly(true)->keepAspectRatio(true)->keepFrame(true)
                ->setImageFile($imageFile)
                ->getUrl(),
            'small' => $this->imageHelper->init($product, 'product_swatch_image_small')
