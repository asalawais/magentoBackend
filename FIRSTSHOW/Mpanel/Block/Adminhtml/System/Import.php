 ...
 */
namespace FIRSTSHOW\Mpanel\Block\Adminhtml\System;

use Magento\Framework\App\Filesystem\DirectoryList;
class Import extends \FIRSTSHOW\Mpanel\Block\Adminhtml\System\Install
{
            ...
            if ($dh = opendir($this->_dir)) {
				...
				foreach($dirs as $theme){
					if(($theme !='') && ($theme!='.') && ($theme!='..')){
						...
						$html .= '<div>';
						$html .= '<div class="section-config"><div class="entry-edit-head admin__collapsible-block"><span class="entry-edit-head-link" id="firstshowtheme_import_'.$theme.'-link"></span><a onclick="Fieldset.toggleCollapse(\'firstshowtheme_import_'.$theme.'\', \''.$this->getUrl('adminhtml/system_config/state').'\'); return false;" href="#firstshowtheme_import_'.$theme.'-link" id="firstshowtheme_import_'.$theme.'-head">'.$themeName.'</a></div><input type="hidden" value="0" name="config_state[firstshowtheme_import_'.$theme.']" id="firstshowtheme_import_'.$theme.'-state"><fieldset id="firstshowtheme_import_'.$theme.'" class="config admin__collapsible-block" style="display:none"><legend>'.$themeName.'</legend>';
						
						$themeDir = $this->_filesystem->getDirectoryRead(DirectoryList::APP)->getAbsolutePath('code/FIRSTSHOW/Mpanel/data/themes/'.$theme.'/homes');
								...
								if(count($fileHomes)>0){
									...
									foreach ($fileHomes as $fileHome){
										...
										if(isset($file_parts_home['extension']) && $file_parts_home['extension']=='xml'){
											...
											$html .= '<img src="'.$this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]).'firstshow/'.$theme.'/homes/'.$home.'.png" width="250"/>';
											$html .= '</td><td>';
						...
						$html .= '</fieldset><script type="text/javascript">//<![CDATA[require([\'prototype\'],function(){Fieldset.applyCollapse(\'firstshowtheme_import_'.$theme.'\');});//]]></script></div>';
