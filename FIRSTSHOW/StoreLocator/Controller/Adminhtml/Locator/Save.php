...
class Save extends \Magento\Backend\App\Action {
    ...
    public function execute() {
        ...
        $model = $this->_objectManager->create('FIRSTSHOW\StoreLocator\Model\Store');
        if (!empty($data['id'])) {
            $model->load($data['id']);
        }
        ...
        if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
            ...
            if($uploader->checkAllowedExtension($ext)) {
                $path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)
                        ->getAbsolutePath('firstshow_storelocator/');
                $uploader->save($path);

                $fileName = $uploader->getUploadedFileName();
                if ($fileName) {
                    $data['image'] = 'firstshow_storelocator'.$fileName;
                }
            } else {
    ...
    /**
     * @param FIRSTSHOW\StoreLocator\Model\StoreFactory $model
     * @param array $data
     * @return void
     */
